...
% calculate goals from angle
   ...
   % calculate mu
    ...
    elseif (120 < run) && (run <= 240)
        mu = 0.09;
    elseif (240 < run) && (run <= 360)
        mu = 1;
    elseif (360 < run) && (run <= 480)
        mu = 0.05;
    elseif 480 < run
        mu = 0.5;
    end
    ...
    try
        ...
        for k = 1:length(x)
            ...
            if min(d_error) > max_dist
                max_dist = min(d_error);
            end
        end
    ...
    % any failure inside the thresholds loop will trigger this catch
    % this is not ideal because we will not know which threshold failed
    % maybe use two try/catches? one for the bagfile data and one for the
